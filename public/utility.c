...
void util_utc_sec_to_bcdtime_base2000(time_t sec_time, u8 *bcd, u8 zone)
{
    ...
    bcd[0] = HEX2BCD(tm_t.tm_year + 1900 - 2000);
    bcd[1] = HEX2BCD(tm_t.tm_mon + 1);
    bcd[2] = HEX2BCD(tm_t.tm_mday);
    bcd[3] = HEX2BCD(tm_t.tm_hour);
    bcd[4] = HEX2BCD(tm_t.tm_min);
    bcd[5] = HEX2BCD(tm_t.tm_sec);
...
{
	KK_CHANGE_ENUM ret = KK_NO_CHANGE;
...
KK_ERRCODE ConvertIpAddr(u8 *addr , u8 *ip)
{
    ...
    if(sscanf((const char *)addr,"%d.%d.%d.%d",p,p+1,p+2,p+3) == 4)
    {
        ip[0] = (u8)p[0];
        ip[1] = (u8)p[1];
        ip[2] = (u8)p[2];
...
KK_ERRCODE util_convertipaddr(u8 *addr , u8 *ip)
{
    ...
    if(sscanf((const char *)addr,"%d.%d.%d.%d",d,d+1,d+2,d+3) == 4)
    {
        ip[0] = (u8)d[0];
        ip[1] = (u8)d[1];
        ip[2] = (u8)d[2];

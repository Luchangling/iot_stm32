...
s32 kk_os_semaphore_request(s32 sem_id , u32 time_out)
{
    ...
    #else
    OSSemPend((OS_EVENT *)sem_id,time_out/KK_OS_TICKS,&err);
    if(err == OS_ERR_NONE) return KK_SUCCESS;
    #endif
...
void kk_os_semaphore_clear(s32 sem_id)
{
    ...
    #else
    OSSemSet((OS_EVENT *)sem_id,0,&err);
    #endif
}
...
void kk_os_semaphore_destory(s32 sem_id)
{
    ...
    #else
    OSSemDel((OS_EVENT *)sem_id,0,&err);
    #endif
}
#if 1
/*********************************************************************************************
...
s32 kk_os_mutex_create(void)
{
    ...
    id = 0;//kk_os_search_idle_event_tbl();
    #if 0
    if(id >= 0)
    {
...
s32 kk_os_msg_queue_create(s32 max_q_count)
{
    ...
    //if(id >= 0)
    {
        s_kk_os_queue_node[0] = (void *)malloc(max_q_count * sizeof(u32));

        if(!s_kk_os_queue_node[0]) return KK_MEM_NOT_ENOUGH;
        
        id = (s32)OSQCreate(&s_kk_os_queue_node[0],(u16)max_q_count);
    }
...
s32 kk_os_send_msg(s32 que_id ,void *msg)
{
    ...
    {
        if(((err = OSQPost((OS_EVENT *)que_id,msg)) != OS_ERR_NONE)  && i < 1000)
        {
            i++;
            kk_os_sleep(10);
        }
...
void *kk_os_recive_msg(s32 que_id , KKOSMsgEnum is_pend)
{
    ...
    //if(s_kk_os_event_pointer[que_id])
    {
        ...
        {
            ptr =  OSQAccept((OS_EVENT *)que_id,&err);
        }
        else
        {
            ptr =  OSQPend((OS_EVENT *)que_id,20,&err);
        }
    }
...
void kk_os_queue_destory(s32 que_id)
{
    ...
    //if(s_kk_os_event_pointer[que_id])
    {
        OSQDel((OS_EVENT *)que_id,01,&err);
    }

    //s_kk_os_event_pointer[que_id] = 0;

    free(s_kk_os_queue_node[0]);

    s_kk_os_queue_node[0]    = 0;
}

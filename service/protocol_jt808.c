...
static Jt08MsgSave s_jt_msg_save = {0, false, 0};

static KK_ERRCODE protocol_jt_pack_unescape(u8 *src, u16 *src_len);
...
void protocol_jt_pack_escape(u8 *src, u16 src_len, u8 *dest, u16 *dest_len)
{
    ...
    if((*dest_len) < j + 1)
    {
        LOG(LEVEL_INFO,"protocol_jt_pack_escape assert(dest_len(%d) >= %d) failed.",  *dest_len, j + 1);
        return;
    }
    dest[j] = src[i];
    j++;
    ...
    for(i = 1; i<src_len - 1; ++i)
    {
        if(src[i] == PROTOCOL_HEADER_JT808)
        {
            if((*dest_len) < j + 2)
            {
                LOG(LEVEL_INFO,"protocol_jt_pack_escape assert(dest_len(%d) >= %d) failed.",  *dest_len, j + 2);
                return;
            }
...
static KK_ERRCODE protocol_jt_pack_unescape(u8 *src, u16 *src_len)
{
    ...
    for(i = 1,j=1; i< (*src_len - 1); ++i,++j)
    {
        if(src[i] == PROTOCOL_HEADER_ESCAPE)
        {
            ...
            {
                LOG(LEVEL_INFO,"protocol_jt_pack_unescape assert(%02x after 0x7d) failed.",  src[i + 1]);
...
static void protocol_jt_pack_head(u8 *pdata, u16 *idx, u16 len, u16 total, u16 part_num )
{
    ...
    if((*idx) + 13 > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_head assert(len(%d)) failed.",  len);
        return;
    }
    ...
    {
        LOG(LEVEL_INFO,"protocol_jt_pack_head can not get imei.");
    }
    else
    {
        strcpy((char *)imei,ModelImei);
    }
    
    if (0 == strlen((const char *)imei))
    {
	...
	pdata[(*idx)++] = MERGEBCD(0, util_chr(imei[4]));
...
static void protocol_jt_pack_id_len(u8 *pdata, u16 *idx, u16 cmd, u16 len, u8 sec, bool depart)
{
    ...
    attribute = len & 0x3FF + (((u16)flag) << 13) + (((u16)(sec & 0x7)) << 10);
...
static void protocol_jt_pack_register_info(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
	...
	#if 0
    if(at_type == AT_TYPE_CUSTOM)
    {
        copy_len = 20;
        memset(&pdata[(*idx)], 0 , copy_len);
        strncpy((char *)&pdata[(*idx)],config_service_get_device_type(value_u16), copy_len);
        (*idx) += copy_len;
    }
    else
    {
    ...
    copy_len = 8;
    memset(&pdata[(*idx)], 0 , copy_len);
    strncpy((char *)&pdata[(*idx)],"BMS_V", copy_len);
    (*idx) += copy_len;
    #endif
    ...
    copy_len = util_remove_char(value_string, strlen((const char *)value_string),' ');
    memcpy(&pdata[(*idx)], value_string, copy_len);
    (*idx) += copy_len;
...
void protocol_jt_pack_regist_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_regist_msg assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_regist_msg assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
...
static void protocol_jt_pack_auth_code(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_auth_code assert(len(%d)) failed.",  content_len);
        return;
    }
...
void protocol_jt_pack_auth_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_auth_msg assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_auth_msg assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
...
static void protocol_jt_pack_iccid(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    if(strlen((const char*)ModelIccid) == 0)
    {
        LOG(LEVEL_INFO,"protocol_jt_pack_iccid can not get iccid, ret:%d.",  ret);
    }
    ...
    if((*idx) + content_len > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_iccid assert(len(%d)) failed.",  content_len);
        return;
    }
...
void protocol_jt_pack_iccid_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_iccid_msg assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_iccid_msg assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
...
void protocol_jt_pack_logout_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_logout_msg assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_logout_msg assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
...
void protocol_jt_pack_heartbeat_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_heartbeat_msg assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_heartbeat_msg assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
...
static void protocol_jt_pack_gps_info(GpsDataModeEnum mode, const GPSData *gps, u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    LOG(LEVEL_DEBUG,"protocol_jt_pack_gps_info (%d , %d) gps_time(%d:%d:%d) (%02x-%02x-%02x %02x:%02x:%02x).", 
...
static void protocol_jt_pack_additional_imsi(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    if(strlen((const char*)ModelImsi) == 0)
    {
        LOG(LEVEL_INFO,"protocol_jt_pack_imsi can not get imsi, ret:%d.",  ret);
    }
    content_len = strlen((const char *)imsi);
    if (0 == content_len)
    {
    ...
    if((*idx) + content_len > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_imsi assert(len(%d)) failed.",  content_len);
        return;
    }

	pdata[(*idx)++] = 0x41;
	pdata[(*idx)++] = content_len;
    memcpy(&pdata[*idx], &imsi[0], content_len);
    (*idx) += content_len;
    *return_len = content_len + 2;
}
#endif
...
static void protocol_jt_pack_alarm_status(u8 *pdata, u16 *idx, u16 len, u16 *return_len)
{
    ...
    if((*idx) + 4 > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_alarm_status assert(len(%d)) failed.",  len);
        return;
    }
...
void protocol_jt_pack_gps_msg(GpsDataModeEnum mode, const GPSData *gps, u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_gps_msg assert(malloc(%d)) failed.",  len);
		return;
	}

    memcpy(&tmp_gps, gps, sizeof(tmp_gps));    
    ...
    {
        if(ModelLat > 0 && ModelLong >0)
        {
            tmp_gps.gps_time = util_get_utc_time();
            tmp_gps.lat = ModelLat;
            tmp_gps.lng = ModelLong;
        }
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_gps_msg assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
...
void protocol_jt_pack_lbs_msg(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_lbs_msg assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_lbs_msg assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
    protocol_jt_pack_id_len(send, &send_len, JT_CMD_LOCATE, content_all, 0, false);  // 2bytes
...
void protocol_jt_parse_msg(u8 *pdata, u16 len)
{
    ...
    if(KK_SUCCESS != protocol_jt_pack_unescape(pdata, &len))
    {
        return;
    }
    ...
    {
        LOG(LEVEL_WARN,"protocol_jt_parse_msg assert(checksum(%02x=%02x, %02x, %02x)) failed.",  check_sum, pdata[len-3], pdata[len-2], pdata[len-1]);
        return;
    }
    ...
    switch(cmd)
    {
        case JT_CMD_PLAT_ACK:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_PLAT_ACK).", util_clock());
            protocol_jt_parse_plat_ack(pdata,len);
            break;
        case JT_CMD_REGISTER_ACK:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_REGISTER_ACK).", util_clock());
            protocol_jt_parse_register_ack(pdata,len);
            break;
        case JT_CMD_SET_PARAM:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_SET_PARAM).", util_clock());
            protocol_jt_parse_set_param(pdata,len);
            break;
        case JT_CMD_GET_PARAM:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_GET_PARAM).", util_clock());
            protocol_jt_parse_get_param(pdata,len);
            break;
        case JT_CMD_TERM_CTL:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_TERM_CTL).", util_clock());
            protocol_jt_parse_terminal_control(pdata,len);
            break;
        case JT_CMD_LOCATE_REQ:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_LOCATE_REQ).", util_clock());
            protocol_jt_parse_gps_req(pdata,len);
            break;
        case JT_CMD_CONFIRM_ALARM:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_CONFIRM_ALARM).", util_clock());
            protocol_jt_parse_general_msg(pdata,len);
            break;
        case JT_CMD_TEXT:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(JT_CMD_TEXT).", util_clock());
            protocol_jt_parse_text(pdata,len);
            break;
        default:
            LOG(LEVEL_DEBUG,"protocol_jt_parse_msg(%04x).", cmd);
            LOG_HEX(LEVEL_WARN,pdata,len,"Unknow");
            protocol_jt_parse_general_msg(pdata,len);
            break;
    }
}
...
static void protocol_jt_parse_text(U8* pdata, u16 len)
{
    ...
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_text assert(len(%d)).",  len);
        return;
    }
    ...
    pOut = malloc(CMD_MAX_LEN + 1);
    if (NULL == pOut)
    {
        LOG(LEVEL_WARN,"protocol_concox_parse_remote_msg assert(malloc(%d)) failed.", CMD_MAX_LEN + 1);
        return;
    }
...
static void protocol_jt_parse_gps_req(U8* pdata, u16 len)
{
    ...
    if(len != (data_start + msg_len + 2))
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_gps_req assert(len(%d)).",  len);
        return;
    }
...
static void protocol_jt_parse_general_msg(U8* pdata, u16 len)
{
    ...
    if(len != (data_start + msg_len + 2))
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_general_msg assert(len(%d)).",  len);
        return;
    }
...
static void protocol_jt_parse_terminal_control(U8* pdata, u16 len)
{
    ...
    if(len != (data_start + msg_len + 2))
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_terminal_control assert(len(%d)).",  len);
        return;
    }
    ...
    {
        case 4: //终端复位
            //ret = hard_ware_reboot(KK_REBOOT_CMD,1);
            LOG(LEVEL_DEBUG,"protocol_jt_parse_terminal_control hard_ware_reboot.", util_clock());
            break;
        case 0x64: //断油电 
            //ret = hard_ware_set_relay(true);
            LOG(LEVEL_DEBUG,"protocol_jt_parse_terminal_control hard_ware_set_relay broke.", util_clock());
            break;
        case 0x65: //通油电
            //ret = hard_ware_set_relay(false);
            LOG(LEVEL_DEBUG,"protocol_jt_parse_terminal_control hard_ware_set_relay good.", util_clock());
            break;
    }
...
static void protocol_jt_parse_get_param(U8* pdata, u16 len)
{
    ...
    if(len != (data_start + msg_len + 2))
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_get_param assert(len(%d)).",  len);
        return;
    }
...
static void protocol_jt_parse_set_param(U8* pdata, u16 len)
{
    ...
    if(len != (data_start + msg_len + 2))
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_set_param assert(len(%d)).",  len);
        return;
    }
    ...
    while(para_idx < data_count)
    {
        ...
        switch(para_id)
        {
            ...
            case JT_PARAM_HEART_INTERVAL:
                ...
                if(value_u16 >= KK_HEARTBEAT_MIN && value_u16 <= KK_HEARTBEAT_MAX)
                {
                    config_service_set(CFG_HEART_INTERVAL, TYPE_SHORT, &value_u16, sizeof(value_u16));
                }
                else
                {
                    LOG(LEVEL_INFO,"protocol_jt_parse_set_param assert(JT_PARAM_HEART_INTERVAL(%d)) failed.",  value_u16);
                }
                break;
            case JT_PARAM_APN:
            ...
            case JT_PARAM_HEART_LOCATE_INTERVAL:
                value_u16 = (u16)get_value_by_length(&pdata[data_start], para_len);
                if(value_u16 >= KK_HEARTBEAT_MIN && value_u16 <= CONFIG_UPLOAD_TIME_MAX)
                {
                    config_service_set(CFG_JT_HBINTERVAL, TYPE_SHORT, &value_u16, sizeof(value_u16));
                }
                else
                {
                    LOG(LEVEL_INFO,"protocol_jt_parse_set_param assert(JT_PARAM_HEART_LOCATE_INTERVAL(%d)) failed.",  value_u16);
                }
                break;
            case JT_PARAM_TIME_INTERVAL:
                ...
                {
                    LOG(LEVEL_INFO,"protocol_jt_parse_set_param assert(JT_PARAM_TIME_INTERVAL(%d)) failed.",  value_u16);
                }
                break;
            case JT_PARAM_TURN_ANGLE:
                ...
                {
                    LOG(LEVEL_INFO,"protocol_jt_parse_set_param assert(JT_PARAM_TURN_ANGLE(%d)) failed.",  value_u16);
                }
                break;
            case JT_PARAM_MAX_SPEED:
                ...
                {
                    LOG(LEVEL_INFO,"protocol_jt_parse_set_param assert(JT_PARAM_MAX_SPEED(%d)) failed.",  value_u8);
                }
                break;
...
static void protocol_jt_parse_register_ack(U8* pdata, u16 len)
{
    ...
    if(len != (data_start + msg_len + 2))
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_register_ack assert(len(%d)).",  len);
        return;
    }
    ...
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_register_ack regist ok(%d).",  pdata[data_start + 2]);
        memset(auth_code, 0x00, sizeof(auth_code));
    ...
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_register_ack regist failed(%d).",  pdata[data_start + 2]);
    }
}
...
static void protocol_jt_parse_plat_ack(U8* pdata, u16 len)
{
    ...
    if(len != (data_start + msg_len + 2))
    {
        LOG(LEVEL_INFO,"protocol_jt_parse_plat_ack assert(len(%d)).",  len);
        return;
    }
    ...
    switch(msg_id)
    {
        case JT_CMD_AUTH:
            ...
            {
                LOG(LEVEL_INFO,"protocol_jt_parse_plat_ack auth ok(%d).",  pdata[4]);
                gps_service_after_login_response();
            }
            else
            {
                u8 value_u8 = 0;
                LOG(LEVEL_INFO,"protocol_jt_parse_plat_ack auth failed(%d).",  pdata[4]);
...
void protocol_jt_pack_remote_ack(u8 *pdata, u16 *idx, u16 len, u8 *pRet, u16 retlen)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_remote_ack assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_remote_ack assert(len(%d)) failed content_len(%d) send_len(%d).", 
             len,content_len, send_len);
        free(send);
        return;
    }
...
void protocol_jt_pack_general_ack(u8 *pdata, u16 *idx, u16 len)
{
	...
	if (send == NULL)
	{
        LOG(LEVEL_INFO,"protocol_jt_pack_general_ack assert(malloc(%d)) failed.",  len);
		return;
	}
    ...
    if((send_len + 2) > len)
    {
        LOG(LEVEL_WARN,"protocol_jt_pack_general_ack assert(len(%d)) failed.",  len);
        free(send);
        return;
    }
